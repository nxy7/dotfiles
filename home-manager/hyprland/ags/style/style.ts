...
const variables = () => [
  ...
  $(
    "widget-bg",
    `transparentize(${t(dark.widget, light.widget)}, ${widget.opacity.getValue() / 100})`,
  ),

  $(
    "hover-bg",
    `transparentize(${t(dark.widget, light.widget)}, ${(widget.opacity.getValue() * 0.9) / 100})`,
  ),
  ...
  $(
    "border-color",
    `transparentize(${t(dark.border, light.border)}, ${border.opacity.getValue() / 100})`,
  ),
  ...
  $(
    ...
    `transparentize(${t(dark.border, light.border)}, ${Math.max((border.opacity.getValue() - 1) / 100, 0)})`,
  ),
  $("popover-padding", `$padding * ${popoverPaddingMultiplier}`),
  $(
    "popover-radius",
    radius.getValue() === 0 ? "0" : "$radius + $popover-padding",
  ),
