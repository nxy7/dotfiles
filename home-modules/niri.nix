...
with lib;
let
  binds = { suffixes, prefixes, substitutions ? { }, }:
    let
      replacer =
        replaceStrings (attrNames substitutions) (attrValues substitutions);
      format = prefix: suffix:
        let
          actual-suffix = if isList suffix.action then {
    ...
    in listToAttrs (pairs prefixes
      (prefix: pairs suffixes (suffix: [ (format prefix suffix) ])));
in {
  imports = [ inputs.niri.homeModules.niri ];

  programs.niri.settings = {
    input.keyboard.xkb.layout = "pl,pl";
    ...
    #   get-wayland-display =
    #     "systemctl --user show-environment | awk -F 'WAYLAND_DISPLAY=' '{print $2}' | awk NF";
    #   wrapper = name: op:
    ...
    #     } ${config.stylix.image} -modulate 100,100,14 $out
    #   '';
    # in [
    #   {
    #     command = [
    ...
    #     ];
    #   }
    #   { command = [ "${only-without-session}" "${lib.getExe pkgs.waybar}" ]; }
    #   {
    #     command = [
    ...
    #         (map (unit: "systemctl --user status ${unit}") units);
    #     in [
    #       "${only-on-session}"
    #       "kitty"
    #       "--"
